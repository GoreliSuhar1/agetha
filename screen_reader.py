...
class ScreenReader:
    ...
    def capture_text(self, max_chars: int = 3000) -> str:
        ...
        try:
            ...
            # Убрали сжатие картинки (resize), чтобы Tesseract не спотыкался о мелкий шрифт
            
            # Добавили поддержку языков eng, rus и ukr через плюс
            text = pytesseract.image_to_string(screenshot, lang="eng+rus+ukr", config="--psm 3")
